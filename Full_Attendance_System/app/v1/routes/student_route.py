...
from app.v1.schemas.student_schemas import StudentCreate, StudentLogin, StudentOut, StudentResetPassword, StudentUpdate
from app.v1.services.auth_services.student_auth_services import StudentAuthService
...
@router.post("/validate_email", response_model=StudentOut)
async def login_student(data: StudentLogin, session: AsyncSession = Depends(get_db)):
...
@router.post("/reset_password", response_model=StudentOut)
